...
public class SeleniumMethodControl {
  ...
  /**
   * This will run the contained method with any arguments that it contains. The returned value depends on whether the method is looking
   ...
   */
    ...
    /**
     * Runs the Selenium method unless it is the isTextPresent method.  The initial assumption is that all parameters used are Strings
     * since Selenium almost solely deals with String inputs (With the exception of the wait commands which are dealt with separately)
   ...
   case -1: log.error( "Method " + index + ", " + method.getName( ) + ", in " + navName + "was not found on " + this.getPageToString( ) );
	  	return false;	 
   case 0: log.error( "Method " + index + ", " + method.getName( ) + ", in " + navName + "did not complete successfully on " + this.getPageToString( ) );
		  return false;
   case 1: log.info( "Method " + index + ", " + method.getName( ) + ", in " + navName + " completed successfully on " + this.getPageToString( ) );
         break;
   case 2: log.info( "Search " + index + ", in " + navName + " could not find the text " + method.getArgumentsToString( ) + " on " + this.getPageToString( ) );
         break;
   case 3: log.info( "Search " + index + ", in " + navName+ " found the text " + method.getArgumentsToString( ) + " on " + this.getPageToString( ) );
         break;
   case 4: log.error( "Method " + index + ", " + method.getName( ) + ", in " + navName + " was not run as it did not contain any arguments. " );
   		return false;
   case 5: log.info( "Wait " + index + ", in " + navName + " completed successfully on " + this.getPageToString( ) );
         break;
   case 6: log.info( "Wait " + index + " with duration, in " + navName + " completed successfully on " + this.getPageToString( ) );
         break;
   case 7: log.error( "Wait " + index + ", in " + navName + " was not run as it did not contain any arguments. " );
   		return false;
   case 8: log.warn( "Wait " + index + ", in " + navName + " was interurrpted but will system will continue. " );
         break;
   case 9: log.error( "Method " + index + ", " + method.getName( ) + ", in " + navName + ": Error: Status not recorded." );
   break;
   default: log.error( "Method " + index + ", " + method.getName( ) + ", in " + navName + ": Unknown outcome ocrrured." );
   		return false;
		   }
    	return true;
